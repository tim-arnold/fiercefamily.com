...
#                                                                            #
# $mailprog defines the location of your sendmail program on your Windows NT #
# system.  Make sure you have permissions to execute sendmail in the         #
# specified directory.  Be sure to escape (\) any \ characters               # 
# (ie. $mailprog = 'c:/winnt/system32/blat.exe')                          #
...
$tempdir = 'd:/209.35.79.215/cgi-bin';
...
$uselog = 1;            # 1 = Yes; 0 = No
$logfile = 'd:/209.35.79.215/cgi-bin/formmail.log';
...
@referers = ('209.35.79.215', 'phenon.com', 'www.phenon.com', 'augustineconsulting.com', 'www.augustineconsulting.com');
...
sub return_html {
	...
	print "<html><head>\n";
	print "<META HTTP-EQUIV=\"REFRESH\" CONTENT=\"0; URL=$Config{'redirect'}\">\n";
...
sub send_mail {
    ...
    open(MAIL,">$tempdir\\tempfile") || die("Cannot open $tempdir\\tempfile -- Check Directory Permissions : $!"); 
    ...
    close (MAIL);
        ...
        Win32::Process::Create($ProcessObj, $mailprog, "Blat $tempdir\\tempfile -t $Config{'recipient'} -s \"$Config{'subject'}\" -f $Config{'email'} -i \"$Config{'email'} ($Config{'realname'})\"", 0, DETACHED_PROCESS, ".")|| die &Error; 
...
#open(MAIL,"|$mailprog $tempdir\\tempfile -t $Config{'recipient'} -s \"$Config{'subject'}\" -i \"$Config{'email'} ($Config{'realname'})\" -f $Config{'email'}") || die("Cannot execute $mailprog -- Check mailprog path and permissions : $!"); 
#close(MAIL);
...
sub log {
   ...
   if ($log_type eq 'entry') {
      print LOG "$ENV{'REMOTE_HOST'} - [$date] - Successful\n";
	print LOG "Recipient: $Config{'recipient'}\n";
	print LOG "Subject: $Config{'subject'}\n";  
	print LOG "Sender: $Config{'email'}($Config{'realname'})\n";
	print LOG "\n";
   }
   elsif ($log_type eq 'missing_feilds') {
      print LOG "$ENV{'REMOTE_HOST'} - [$date] - ERR: Missing Required Feilds\n";
	print LOG "Recipient: $Config{'recipient'}\n";
	print LOG "Subject: $Config{'subject'}\n";  
	print LOG "Sender: $Config{'email'}($Config{'realname'})\n";
	print LOG "\n";
   }
   elsif ($log_type eq 'missing_fields_redirect') {
      print LOG "$ENV{'REMOTE_HOST'} - [$date] - ERR: Missing Feild - Redirect\n";
	print LOG "Recipient: $Config{'recipient'}\n";
	print LOG "Subject: $Config{'subject'}\n";  
	print LOG "Sender: $Config{'email'}($Config{'realname'})\n";
	print LOG "\n";
   }
   elsif ($log_type eq 'no_recipient') {
      print LOG "$ENV{'REMOTE_HOST'} - [$date] - ERR: No Recipient\n";
	print LOG "Recipient: $Config{'recipient'}\n";
	print LOG "Subject: $Config{'subject'}\n";  
	print LOG "Sender: $Config{'email'}($Config{'realname'})\n";
	print LOG "\n";
   }
   elsif ($log_type eq 'bad_refferer') {
